...
public class TestHelper {
     ...
     * @throws Exception
     */
    public static void addDataFromFile(TaskManager taskManager, String pathString) throws Exception {
    ...
    public static Expectations parse(String value){
        ...
        for (String str : split){
            ...
            switch (data[0].trim().toLowerCase()){
                case "status" : expectations.status =data[1].trim(); break;
                case "duration": expectations.duration = Integer.parseInt(data[1].trim());break;
                case "start_time": expectations.startTime=data[1].trim(); break;
                case "end_time": if(data[1].trim().toLowerCase().equals("null")) expectations.isNullEndTime = true;
                else expectations.endTime=data[1];break;
                default:
            }
        }
        return expectations;
